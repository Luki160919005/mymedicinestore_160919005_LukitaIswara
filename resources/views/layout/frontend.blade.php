                    ...
                    <div class="row total-header-section">
                        ...
                        @if(session()->get('cart'))
                            @foreach(session()->get('cart') as $id =>$details)
                                <?php 
                                    $tot+= $details['price']*$details['quantity']; 
                                ?>
                            @endforeach
                        @endif
                        <div class="col-lg-6 col-sm-6 col-6 total-section text-right">
                            <p>Total: <span class="text-info">{{$tot}}</span></p>
                        </div>
                    </div>
                    @if(session('cart')) 
                        ...
                        <div class="row cart-detail">
                            ...
                            <div class="col-lg-8 col-sm-8 col-8 cart-detail-product">
                                <p>{{$details['name']}}</p>
                                <span class="price text-info"> {{$details['price'] * $details['quantity']}}</span> <span class="count"> Quantity:{{$details['quantity']}}</span>
                            </div>
                        </div>
                        @endforeach 
                    @endif
